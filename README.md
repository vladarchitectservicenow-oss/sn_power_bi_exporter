# sn_power_bi_exporter
Author: Vladimir Kapustin

## Architecture
```mermaid
graph TD
    SN[ServiceNow Instance] -->|REST| sn_power_bi_exporter
    sn_power_bi_exporter -->|Store| DB[ServiceNow Tables]
    sn_power_bi_exporter -->|Output| Report[MD/JSON Reports]
```
## Installation
```bash
git clone https://github.com/vladarchitectservicenow-oss/sn_power_bi_exporter.git
cd sn_power_bi_exporter
python3 src/cli.py --sn-url https://dev.instance.com --help
```
## ROI Calculator
| Metric | Manual | With sn_power_bi_exporter |
|--------|--------|-------------|
...
# Run scan
POST /api/x_sn_power_bi_exporter/scan
```
## Security & Compliance
...
## Overview
sn_power_bi_exporter is a production-grade ServiceNow scoped application developed by Vladimir Kapustin under AGPL-3.0.

## Architecture
```mermaid
graph TD
    SN[ServiceNow Instance] -->|REST| sn_power_bi_exporter
    sn_power_bi_exporter -->|Store| DB[x_sn_power_bi_exporter_tables]
    sn_power_bi_exporter -->|Output| Report[Reports MD/JSON/CSV]
...
## ROI Analysis
| Metric | Manual Process | With sn_power_bi_exporter |
|--------|---------------|-------------|
...
See `Validation/TEST CASES/sn_power_bi_exporter/test_suite_SOP.md`
...
## Support
- GitHub Issues: https://github.com/vladarchitectservicenow-oss/sn_power_bi_exporter/issues
- ServiceNow Community: Tag `sn_power_bi_exporter`


---

## Detailed Product Overview
sn_power_bi_exporter is a production-ready scoped application engineered specifically for the ServiceNow platform, targeting enterprises that demand high automation, rigorous compliance, and deep observability. It bridges critical gaps in native ServiceNow capabilities by providing custom business logic, reusable Script Includes, and robust REST endpoints that integrate with external DevOps, security, analytics, and collaboration tools.
...
sn_power_bi_exporter is packaged using the ServiceNow Application Repository and Studio standards. It ships with a complete `sys_app` definition, scoped tables, Business Rules, Script Includes, Client Scripts, UI Policies, UI Actions, scheduled jobs, and inbound REST APIs. The application follows ServiceNow naming conventions: the scope is `x_sn_power_bi_exporter`, and all artifacts are prefixed consistently to avoid collisions with other applications.

In large organizations, manual execution of the workflows handled by sn_power_bi_exporter typically consumes 40–120 hours per year per team. At a blended hourly rate of $85/hour, this translates to $3,400–$10,200 annually in labor cost, not including opportunity cost, human error, or compliance penalties. sn_power_bi_exporter reduces this workload to fewer than 5 hours per year through automation, delivering an 87%+ cost reduction with immediate payback.
...
4. Enter the repository URL: `https://github.com/vladarchitectservicenow-oss/sn_power_bi_exporter.git`.
...
|----------|------|---------|-------------|
| `x_sn_power_bi_exporter.logging.level` | Choice | INFO | DEBUG, INFO, WARN, ERROR |
| `x_sn_power_bi_exporter.scan.chunk_size` | Integer | 500 | Records per batch for large tables |
| `x_sn_power_bi_exporter.scan.delta.enabled` | Boolean | true | Enable incremental scanning |
| `x_sn_power_bi_exporter.output.format.default` | Choice | md | Default report format |
| `x_sn_power_bi_exporter.api.timeout_seconds` | Integer | 30 | REST call timeout |
| `x_sn_power_bi_exporter.encryption.key_id` | String | — | Reference to sys_encryption_context |
| `x_sn_power_bi_exporter.healthcheck.enabled` | Boolean | true | Enable health endpoint |

Roles:
- `x_sn_power_bi_exporter.admin`: Full configuration, deployment, and report access.
- `x_sn_power_bi_exporter.user`: Read-only report viewing and health-check access.
- `x_sn_power_bi_exporter.api`: Service account role for CI/CD pipelines.
...
| Memory exhaustion | Chunk size too large for JVM/Python heap | Reduce `x_sn_power_bi_exporter.scan.chunk_size` | Tune heap per instance sizing |
...
## Support & Community
- **GitHub Issues:** https://github.com/vladarchitectservicenow-oss/sn_power_bi_exporter/issues
- **Discussions:** https://github.com/vladarchitectservicenow-oss/sn_power_bi_exporter/discussions
- **ServiceNow Community:** Tag `sn_power_bi_exporter` in posts.

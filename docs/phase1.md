# Phase 1: Discovery & Design — Power Bi Exporter

## Scope
This document captures the requirements-gathering, architectural design, and planning phase for sn_power_bi_exporter.
...
- `x_sn_power_bi_exporter_config`: Application configuration and properties.
- `x_sn_power_bi_exporter_scan_log`: Record of each scan execution (start, end, status, record count).
- `x_sn_power_bi_exporter_report`: Generated reports with metadata (format, size, checksum).
- `x_sn_power_bi_exporter_delta_watermark`: High-water marks for incremental scanning per table.
...
| R3 | Scope collision with other apps | Medium | Low | Unique prefix `x_sn_power_bi_exporter`; validate before install |
